...
const Scene = ({
  ...
  useEffect(() => {
    ...
    const accessToken = localStorage.getItem(
      process.env.NEXT_PUBLIC_ACCESSTOKENKEY
    )
    if (!accessToken) {
  ...
  return (
    <>
      ...
      {!isMobile && (
        <Drawer isOpen={drawer.isOpen} onClose={closeDrawer}>
          {drawer.iframeSrc ? (
            <iframe
              title=""
              width="100%"
            ...
            <div className="w-full h-full bg-black bg-opacity-60 flex flex-col justify-center items-center overflow-y-auto">
              {drawer.imgList.map((item) =>
                isDuringDate(item.startAt, item.endAt) ? (
                  <img className="w-full mt-2" key={item.id} src={item.src} />
                ) : null
              )}
            </div>
          )}
      ...
      <div className="z-50 fixed top-5 left-5 text-xl ym-text-blue font-bold">
        https://vhq.yomo.run
      </div>
    </>
  )
