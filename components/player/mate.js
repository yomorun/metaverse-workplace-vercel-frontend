...
const Mate = ({ name, avatar, initPos, socket }) => {
    const refContainer = useRef(null)
    const trackMap = useRecoilValue(trackMapState)
    const { videoTrack, audioTrack } = trackMap.get(name) || { videoTrack: null, audioTrack: null }
    ...
    useEffect(() => {
        ...
        const subscription = direction$
            .pipe(scan((currPos, dir) => currPos.add(dir), POS))
            .subscribe(currPos => {
